(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Romania-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Romania-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Romania
                                       ...
                                       Grounded in EU establishment or Romanian
                                       CUI entity for many procedures
                                       (contracting authorities / ANAP).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CUI/統一資格/SEAP/SICAP登録/代理人確認等)が充足していない状態での提案"}]))))
...
(defn- eu-establishment-missing-violations
  ...
  `:requires-eu-establishment?` ground truth (most Romania public
    ...
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
        ...
        [{:rule :eu-establishment-missing
          :detail (str subject " はルーマニア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- cui-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-cui? true`, INDEPENDENTLY check
  `:cui-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-cui? e))
                 (not (true? (:cui-verified? e))))
        [{:rule :cui-unverified
          :detail (str subject " はCUI確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (cui-unverified-violations request st)
